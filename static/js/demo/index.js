...
export const DEMO_SYNTHESIS = {
  ...
  // 퍼널 단계별 그룹 LLM 인사이트 (quant_groups 순서와 동일)
  upper_group_insights: [
    '브랜드 인지·태도 그룹(평균 3.2)은 브랜드 호감도(3.5)에 비해 브랜드 적합성(2.9)·신뢰도(3.1)가 낮아 점수를 끌어내리고 있다. 패널들이 "브랜드 자체는 좋지만 이 프로모션이 그 이미지와 맞는지 모르겠다"고 언급한 것처럼, 기존 브랜드 자산과 프로모션 간의 핏 부재가 핵심 원인이다.',
    '광고 효과성 그룹(평균 3.1)에서 주목도(3.8)가 메시지 명확성(2.4)보다 1.4점 높게 나타났다. 비주얼이 눈길을 끄는 데는 성공했지만 "무엇을 말하는지 모르겠다"는 패널 반응이 다수였다. 감성 이미지에 치우쳐 핵심 오퍼와 가치 메시지가 누락된 소재 구조 문제다.',
  ],
  mid_group_insights: [
    '가치 인식 그룹(평균 2.8)에서 가격 적정성(2.3)이 매력도(3.2)보다 현저히 낮다. "끌리기는 하지만 이 가격을 쓸 만큼인지 모르겠다"는 패턴으로, 워킹맘·절약형 페르소나에서 집중적으로 나타났다. 감성 소구만으로는 가치 정당화가 불가하며 구체적 혜택 증거가 필요하다.',
    '구전·정보 그룹(평균 2.7)에서 정보 충분성(2.5)이 낮은 것이 추천 의향(2.9) 형성을 막는 선행 장벽이다. 패널들이 "추천하려해도 구체적으로 뭐가 좋은지 설명하기 어렵다"고 언급한 것처럼, 정보 부재가 WOM 경로를 차단하고 있다.',
  ],
  lower_group_insights: [
    '구매 의향 그룹(평균 2.4)에서 구매 의향(2.2)이 구매 가능성(2.5)·고려 확률(2.6)보다 낮다. "마음은 있지만 지금 당장 살 이유는 없다"는 상태로, MZ세대가 "인플루언서 추천이 있으면 바로 살 것 같다"고 언급한 것처럼 외부 전환 트리거가 없는 상태에서는 의향이 행동으로 전환되지 않는 구조다.',
    '재구매·시급성 그룹(평균 2.1)이 전체에서 가장 낮다. 구매 시급성(2.0)이 특히 낮은 것은 프로모션이 "지금 사야 할 이유"를 만들어내지 못하기 때문이다. 한정 기간·수량 등 긴급성 장치가 없어 결정이 미뤄지고, 재구매 의향(2.3)도 초기 가치 증명이 약한 상태에서는 형성되기 어렵다.',
  ],
  // 퍼널 전체 정량 지표 종합 해석
  upper_quant_insight: '브랜드 퍼널 정량 평균은 3.2/5로 보통 수준이나, 주목도(3.8)와 메시지 명확성(2.4) 사이에 1.4점 격차가 존재해 "눈길은 끌지만 무엇을 말하는지 모르겠다"는 구조적 문제를 드러낸다. 특히 MZ세대는 비주얼 호감(4.0)을, 절약형 패널은 브랜드 적합성(2.6)을 극단적으로 다르게 평가해 세그먼트 간 편차가 뚜렷하다.',
  mid_quant_insight: '수요 창출 퍼널 정량 평균은 2.8/5로 전반적으로 부진하며, 가격 적정성(2.3)이 매력도(3.2)에 비해 현저히 낮아 감성 소구와 지각된 가치 사이의 괴리가 핵심 문제다. "끌리지만 돈 쓸 이유를 모르겠다"는 반응이 절약형·트렌드세터를 제외한 전 세그먼트에서 공통적으로 나타나 소재 내 혜택 증거 부재가 지표를 끌어내리고 있다.',
  lower_quant_insight: '전환 퍼널 정량 평균은 2.3/5로 세 퍼널 중 가장 낮으며, 구매 시급성(2.0)이 고려 확률(2.6)보다 0.6점 낮아 관심은 있으나 행동으로 이어지지 않는 전형적 이탈 구조를 보인다. MZ세대(구매 의향 2.9)와 중장년층(1.8)의 편차가 크게 나타나 동일 소재로 전 세그먼트를 전환시키기 어려운 상황이다.',
};
