...
class Review:
    ...
    def from_llm_response(cls, persona_id: str, persona_name: str, response_text: str) -> "Review":
        try:
            ...
            # Fallback to {} (not data) so missing sections produce clear 0/empty values.
            _norm = {"".join(k.lower().split()): v for k, v in data.items() if isinstance(v, dict)}
            base = _norm.get("기본평가") or {}
            ba   = _norm.get("브랜드태도") or {}
            pv   = _norm.get("지각된가치") or {}
            bf   = _norm.get("브랜드적합성") or {}
            ae   = _norm.get("광고효과성") or {}
            etc  = _norm.get("기타정량") or {}
            pi   = _norm.get("구매의향") or {}
            pp   = _norm.get("구매확률") or {}
            qc   = _norm.get("정성적코멘트") or {}
            ...
            # Parse QA fields if present — same normalization applied to QA section key.
            qa_fields = _norm.get("qa검증") or _norm.get("qa검증항목") or {}
            qa_result = None
